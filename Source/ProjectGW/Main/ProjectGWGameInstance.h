...
public:
	...
	UPROPERTY()
	UDispatchManager* DispatchManager; // 캐릭터 매니저 포인터
	...
	UPROPERTY()
	USpellManager* SpellManager; // 캐릭터 매니저 포인터
	...
	UDispatchManager* GetDispatchManager() const { return DispatchManager; } // 캐릭터 매니저를 반환하는 메소드
	UFUNCTION(BlueprintCallable)
	UCharacterManager* GetCharacterManager() const { return CharacterManager; } // 캐릭터 매니저를 반환하는 메소드
	UFUNCTION(BlueprintCallable)
	USpellManager* GetSpellManager() const { return SpellManager; } // 캐릭터 매니저를 반환하는 메소드
};
